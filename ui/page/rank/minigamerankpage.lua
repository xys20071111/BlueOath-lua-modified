local MinigameRankPage = class("ui.page.Rank.MinigameRankPage")
local offset = 0.01

function MinigameRankPage:initialize()
  self.tabRankInfo = {}
  self.pre = nil
  self.flag = false
end

function MinigameRankPage:DoOnOpen(page, param, widgets)
  self.page = page
  self.param = param
  ...
  UIHelper.SetLocText(self.tab_Widgets.tx_time_title, 4100017)
  self.chapterId = Logic2d:GetChapterId()
  local widgets = self.tab_Widgets
  widgets.Dropdown:ClearOptions()
  local config_all = configManager.GetData("config_minigame_infinite_copy")
  local chapterNameTbl = {}
  for chapterId, config in ipairs(config_all) do
    table.insert(chapterNameTbl, config.name)
    if chapterId == self.chapterId then
      UIHelper.AddDDValue(widgets.Dropdown, chapterId)
      UIHelper.SetDDCaptionText(widgets.Dropdown, config.name)
    end
  end
  UIHelper.AddDDOptions(widgets.Dropdown, chapterNameTbl)
  widgets.Dropdown.onValueChanged:AddListener(function(index)
    local config = config_all[index + 1]
    self.chapterId = config.id
    Logic2d:SetChapterId(config.id)
    self:GetRankData()
  end)
  local config = config_all[1]
  self.chapterId = config.id
  Logic2d:SetChapterId(config.id)
  self:GetRankData()
end

function MinigameRankPage:RegisterAllEvent()
  eventManager:RegisterEvent(LuaEvent.GetRank2d, self.UpdatePage, self)
  self.tab_Widgets.ScrollbarVer.onValueChanged:AddListener(function(msg)
    self:_OnScrollRectChange(self, msg)
  end)
end

function MinigameRankPage:UnRegisterAllEvent()
  eventManager:UnregisterEvent(LuaEvent.GetRank2d, self.UpdatePage)
end

function MinigameRankPage:_OnScrollRectChange(go, volume)
...
function MinigameRankPage:GetRankData()
  local parameter = configManager.GetDataById("config_parameter", 224).arrValue
  local numMaxOnce = parameter[1]
  local args = {
    Start = 0,
    End = numMaxOnce,
    ChapterId = Logic2d:GetChapterId()
  }
  self.flag = true
  Service.userService:GetMiniGameScoreRank(args)
end

function MinigameRankPage:UpdatePage(rankData)
  self:ShowExRank(rankData)
  self:ShowSelfRank(rankData)
  self.flag = false
end

function MinigameRankPage:ShowExRank(rankData)
  self.tabRankInfo = {}
...
function MinigameRankPage:ShowSelfRank(rankData)
  local widgets = self.tab_Widgets
...
function MinigameRankPage:DoOnHide()
end

function MinigameRankPage:DoOnClose()
end

return MinigameRankPage
